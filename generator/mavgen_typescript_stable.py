...
def generate_preamble(f, filelist):
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    src_files = ", ".join(filelist)
    f.write(
        f"""
/*
MAVLink protocol TypeScript implementation (autogenerated).
...
export class BaseMavCmd {
    ...
    for entry in e.entry:
        ...
        f.write("\tconstructor(")
        f.write(
            ", ".join(f"{format_parameter(name)}: number" for name, _, _ in parameters)
        )
        f.write(") {\n")
